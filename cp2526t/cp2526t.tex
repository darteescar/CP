...
pela função \ensuremath{\Varid{concat}}, já na função \ensuremath{\Varid{bfsLevels}}.
...
Neste caso o passo final seria colocar o valor da raiz \ensuremath{\mathrm{5}} (\ensuremath{[\mskip1.5mu \mathrm{5}\mskip1.5mu]}) no início da lista, 
e depois juntar as listas dos níveis das subárvores esquerda e direita,
o que se faz com a função \ensuremath{\Varid{zipWith}\;(\mathbin{+\!\!+})}, que concatena as listas correspondentes de duas listas de listas.
...
\\
\>[B]{}\hsindent{6}{}\<[6]%
\>[6]{}\mathbf{where}\;\Varid{f}\;(\Varid{root},(\Varid{l},\Varid{r}))\mathrel{=}[\mskip1.5mu \Varid{root}\mskip1.5mu]\mathbin{:}\Varid{zipWith}\;(\mathbin{+\!\!+})\;\Varid{l}\;\Varid{r}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
com a função \ensuremath{\Varid{zipWith}\;(\mathbin{+\!\!+})}.
...
em \ensuremath{\Conid{BTrees}}, usando uma floresta, ao invés de uma árvore, para representar os nós a visitar
...
\centerline{
     ...
     \\
          \ensuremath{{(\Conid{BTree}\;\Conid{A})}^{*}}
               ...
               \ar@/_1pc/[r]^-{\ensuremath{\Varid{geneBF}}}
     &
