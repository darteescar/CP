...
onde \ensuremath{\Varid{num}\;\Varid{x}\;\Varid{n}} calcula o numerador \ensuremath{\Varid{x}\mathbin{\uparrow}(\mathrm{2}\mathbin{*}\Varid{n}\mathbin{+}\mathrm{1})} e \ensuremath{\Varid{den}\;\Varid{x}\;\Varid{n}} calcula o denominador \ensuremath{{(\mathrm{2}\mathbin{*}\Varid{n}\mathbin{+}\mathrm{1})!}}.
...
por isso, é necessário a simplificar, até não dependa de \ensuremath{\Varid{n}}. Para isso, faça-se
...
\\
\>[B]{}\hsindent{6}{}\<[6]%
\>[6]{}(\Varid{s},\anonymous ,\anonymous ,\anonymous ,\anonymous )\mathrel{=}\Varid{aux}\;\Varid{x}\;\mathrm{0}{}\<[E]%
\\
\>[B]{}\hsindent{6}{}\<[6]%
...
Trocando o nome de algumas funções, temos o \ensuremath{\Varid{loop}} como:
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
...
E embora a sua definição de não seja dada explicitamente, o seu comportamento pode ser compreendido observando a analogia direta com o catamorfismo determinístico \ensuremath{\llparenthesis\, \cdot \,\rrparenthesis}.
...
\begin{itemize}
     \item Caso base: a lista é vazia, não há valores intermédios a combinar, e o resultado é obtido aplicando diretamente o \ensuremath{\Varid{gene}} a \texttt{Left ()}.
     \item Caso recursivo: a chamada \ensuremath{\Varid{pcataList}\;\Varid{gene}\;\Varid{xs}} devolve uma distribuição de resultados da cauda. Cada valor desta distribuição deve ser processado pelo \ensuremath{\Varid{gene}} novamente, combinando as probabilidades corretamente.
\end{itemize}
...
\\
\>[6]{}\Varid{pcataList}\;\Varid{gene}\;\Varid{xs}\bind \lambda \Varid{y}\to {}\<[E]%
\\
\>[6]{}\Varid{gene}\;(i_2\;(\Varid{x},\Varid{y})){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
