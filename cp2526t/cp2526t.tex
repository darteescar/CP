...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{fair\char95 merge'}\mathbin{::}(\Conid{Stream}\;\Conid{A},\Conid{Stream}\;\Conid{A})+(\Conid{Stream}\;\Conid{A},\Conid{Stream}\;\Conid{A})\to \Conid{Stream}\;\Conid{A}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{geneFM}\mathbin{::}(\Conid{Stream}\;\Varid{a},\Conid{Stream}\;\Varid{a})+(\Conid{Stream}\;\Varid{a},\Conid{Stream}\;\Varid{a})\to \Varid{a}\times((\Conid{Stream}\;\Varid{a},\Conid{Stream}\;\Varid{a})+(\Conid{Stream}\;\Varid{a},\Conid{Stream}\;\Varid{a})){}\<[E]%
\\
...
\begin{itemize}
    \item \textbf{Lista vazia:} representada por \ensuremath{i_1\;()}. Este caso corresponde ao fim da lista, permitindo decidir probabilisticamente se a transmissão termina corretamente ou se ocorre a falha de envio do "stop". 
    \item \textbf{Lista não vazia:} representada por \ensuremath{i_2\;(\Conid{String},[\mskip1.5mu \Conid{String}\mskip1.5mu])}, onde $x$ é a cabeça da lista (palavra atual) e $y$ é o resultado já processado da cauda. Este caso permite combinar a palavra atual com todos os resultados possíveis da cauda, aplicando as regras probabilísticas definidas.
\end{itemize}
