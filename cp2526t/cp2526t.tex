...
Neste caso o passo final seria colocar o valor da raiz \ensuremath{\mathrm{5}} (\ensuremath{[\mskip1.5mu \mathrm{5}\mskip1.5mu]}) no início da lista final, 
e depois juntar as listas dos níveis das subárvores esquerda e direita,
o que se pode fazer com a função \ensuremath{\Varid{zipWith}\;(\mathbin{+\!\!+})}, que concatena as listas correspondentes de duas listas de listas.

Desta forma o comportamento do gene \ensuremath{\Varid{glevels}} pode ser representado por este diagrama:
...
Assim sendo o código que define o gene \ensuremath{\Varid{glevels}} é o seguinte:
...
\\
\>[B]{}\hsindent{6}{}\<[6]%
\>[6]{}\mathbf{where}\;\Varid{f}\;(\Varid{a},(\Varid{l},\Varid{r}))\mathrel{=}[\mskip1.5mu \Varid{a}\mskip1.5mu]\mathbin{:}\Varid{zipWith}\;(\mathbin{+\!\!+})\;\Varid{l}\;\Varid{r}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
com a função \ensuremath{\Varid{zipWith}\;(\mathbin{+\!\!+})} do \emph{Prelude}. Esta função pode ser definida como
...
\centerline{
     ...
     &
          \ensuremath{\mathrm{1}\mathbin{+}\mathit C\times{\mathit C}^{*}}
               \ar@/^1pc/[l]^-{\ensuremath{\Varid{inList}}}
     \\
          \ensuremath{{\Conid{A}}^{*}\times{\mathit B}^{*}}
               \ar[u]^-{\ensuremath{\anaList{\Varid{geneZW}}}}
               \ar@/_1pc/[r]_-{\ensuremath{\Varid{geneZW}}}
     &
          \ensuremath{\mathrm{1}\mathbin{+}\mathit C\times({\Conid{A}}^{*}\times{\mathit B}^{*})}
               \ar[u]_{\ensuremath{\Varid{id}\mathbin{+}\Varid{id}\times\anaList{\Varid{geneZW}}}}
     }
}
\end{eqnarray*}
...
\\
\>[B]{}\Varid{zipWith'}\;\Varid{f}\mathrel{=}\overline{\anaList{\Varid{geneZW}}}\;\mathbf{where}{}\<[E]%
\\
\>[B]{}\hsindent{6}{}\<[6]%
\>[6]{}\Varid{geneZW}\;([\mskip1.5mu \mskip1.5mu],\anonymous )\mathrel{=}i_1\;(){}\<[E]%
\\
\>[B]{}\hsindent{6}{}\<[6]%
\>[6]{}\Varid{geneZW}\;(\anonymous ,[\mskip1.5mu \mskip1.5mu])\mathrel{=}i_1\;(){}\<[E]%
\\
\>[B]{}\hsindent{6}{}\<[6]%
\>[6]{}\Varid{geneZW}\;((\Varid{a}\mathbin{:}\Varid{as}),(\Varid{b}\mathbin{:}\Varid{bs}))\mathrel{=}i_2\;(\Varid{f}\;\Varid{a}\;\Varid{b},(\Varid{as},\Varid{bs})){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
O mónade \ensuremath{\fun{Dist}} faz exatamente isso:
...
\ensuremath{\fun{Dist}} forma um mónade porque:
...
Esta definição permite combinar automaticamente as probabilidades de múltiplas etapas de cálculo. Por exemplo, ao percorrer uma lista de palavras, o mónade \ensuremath{\fun{Dist}} calcula todas as combinações possíveis de palavras transmitidas e perdidas, multiplicando as probabilidades de cada passo sem necessidade de loops ou cálculos manuais. Assim, alterações no comportamento do \ensuremath{\Varid{gene}} propagam-se corretamente em toda a lista, sem haver necessidade de mudar nada mais do que as probabilidades definidas.

No contexto do nosso problema, o mónade \ensuremath{\fun{Dist}} é particularmente útil porque permite que a função \ensuremath{\Varid{pcataList}}:

\begin{itemize}
    \item percorra a lista de palavras,  
    \item aplique probabilisticamente o \ensuremath{\Varid{gene}} a cada elemento, e  
...
\begin{itemize}
    \item No caso de lista vazia, \ensuremath{\Varid{gene}} recebe \ensuremath{i_1\;()}, permitindo decidir probabilisticamente se o processo termina.
    \item No caso de lista não vazia, a cauda da lista é processada primeiro recursivamente, produzindo $y$, que é então combinado com a cabeça $x$ através de \ensuremath{\Varid{gene}\;(i_2\;(\Varid{x},\Varid{y}))}.
\end{itemize}
...
O \ensuremath{\Varid{pcataList}} processa a lista recursivamente da seguinte forma:

\begin{enumerate}
    \item \textbf{Processa a cauda:} primeiro aplica-se \ensuremath{\Varid{pcataList}\;\Varid{gene}} à cauda da lista, neste caso ["bye"]. Isto produz uma distribuição de todas as possíveis mensagens resultantes da cauda, considerando as escolhas probabilísticas do \texttt{gene}. Chamemos esse resultado de $Y = pcataList~gene~["bye"]$.
    
    \item \textbf{Combina com a cabeça:} para cada possível resultado $y \in Y$, aplica-se \ensuremath{\Varid{gene}} à cabeça "hi" junto com $y$, ou seja, calcula-se \texttt{gene (Right ("hi", y))}. Este passo combina o efeito probabilístico da cabeça com todos os resultados possíveis da cauda.
\end{enumerate}
