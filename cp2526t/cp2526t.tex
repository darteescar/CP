...
Logo a definição de \ensuremath{\Varid{glevels}} é a seguinte:
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
...
O \ensuremath{\Varid{gene}} descreve o comportamento local do aparelho de telegrafia. Para cada palavra, define quais são os resultados que podem ocorrer e com que probabilidades. Neste caso, deverá calcular o caso da perda de uma palavra ou o caso da perda da palavra final "stop".

O tipo do \ensuremath{\Varid{gene}} é:
...
O uso do \ensuremath{\cdot +\cdot } permite separar de forma natural os dois casos distintos que surgem ao percorrer uma lista:

\begin{itemize}
    \item \textbf{Lista vazia:} representada por \ensuremath{i_1\;()}. Este caso corresponde ao fim da lista, permitindo ao \ensuremath{\Varid{gene}} decidir probabilisticamente se a transmissão termina corretamente ou se ocorre a falha de envio do "stop". 
    \item \textbf{Lista não vazia:} representada por \ensuremath{i_2\;(\Conid{String},[\mskip1.5mu \Conid{String}\mskip1.5mu])}, onde $x$ é a cabeça da lista (palavra atual) e $y$ é o resultado já processado da cauda. Este caso permite ao \ensuremath{\Varid{gene}} combinar a palavra atual com todos os resultados possíveis da cauda, aplicando as regras probabilísticas definidas.
\end{itemize}

Desta forma, o \ensuremath{\Varid{gene}} consegue tratar de forma modular e uniforme tanto o caso base da lista, quanto a transmissão de cada palavra individual, sem que a função de percorrer a lista (\ensuremath{\Varid{pcataList}}) precise de conhecer detalhes do comportamento probabilístico.

O \ensuremath{\Varid{gene}} é útil porque:

\begin{itemize}
    \item Separa o comportamento local do aparelho da recursão da lista, permitindo alterar as probabilidades ou regras sem modificar \ensuremath{\Varid{pcataList}}.
    ...
    \item Quando combinado com \ensuremath{\Varid{pcataList}} e o mónade \ensuremath{\fun{Dist}}, propaga automaticamente todas as probabilidades e combinações possíveis, garantindo que o resultado final contenha a distribuição completa de mensagens.
\end{itemize}
...
\column{9}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[6]{}\Varid{pcataList}\;\Varid{gene}\;[\mskip1.5mu \text{\ttfamily \char34 hi\char34},\text{\ttfamily \char34 hi~again\char34},\text{\ttfamily \char34 bye\char34}\mskip1.5mu]\mathrel{=}\mathbf{do}\;\{\mskip1.5mu {}\<[E]%
\\
\>[6]{}\hsindent{3}{}\<[9]%
...
\\
\>[6]{}\hsindent{3}{}\<[9]%
\>[9]{}\Varid{gene}\;(i_2\;(\text{\ttfamily \char34 hi\char34},\Varid{y}))\mskip1.5mu\}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
     \end{center}
